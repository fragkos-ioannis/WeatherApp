...
import { changeBackgroundImage, updateDateTime } from './utilities/functions.js';
...
elements.searchForm.addEventListener('submit', async (event) => {
    ...
    const geoRequestUrl = `http://localhost:3000/api/geocode?location=${encodeURIComponent(location)}`;
    ...
    try {
        ...
        if (geocodingData.results.length > 0) {
            ...
            const weatherApiUrl = `http://localhost:3000/api/weather?lat=${lat}&lon=${lng}`;
